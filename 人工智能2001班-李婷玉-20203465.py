...
class TextRNNRaw(nn.Module):
    def __init__(self):
        ...
        ##Complete this code
        #遗忘门
        self.W_f1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_f2 = nn.Linear(n_hidden, n_hidden, bias=False)
        #输入门
        self.W_i1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_i2 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_c1 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_c2 = nn.Linear(n_hidden, n_hidden, bias=False)
        #输出门
        self.W_o1 = nn.Linear(emb_size, n_hidden, bias=False)
    ...
    def forward(self, X):
        ...
        for x in X:
            #遗忘门
            f_t = torch.sigmoid(self.W_f1(x) + self.W_f2(h_t) + self.b_f)
            #输入门
            i_t = torch.sigmoid(self.W_i1(x) + self.W_i2(h_t) + self.b_i)
            c_t1 = self.tanh(self.W_c1(x) + self.W_c2(h_t) + self.b_c)
            #记忆更新，遗忘无用信息并过滤噪声信息
            c_t = c_t * f_t + i_t*c_t1
            # 输出门
            o_t = torch.sigmoid(self.W_o1(x) + self.W_o2(h_t) + self.b_o)
            #把c_t转换成状态
