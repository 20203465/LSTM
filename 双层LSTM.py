...
class TextRNNRaw(nn.Module):
    def __init__(self):
        super(TextRNNRaw, self).__init__()
        self.C = nn.Embedding(n_class, embedding_dim=emb_size)
        ...
        ##Complete this code
        #第一层遗忘门
        self.W_f11 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_f12 = nn.Linear(n_hidden, n_hidden, bias=False)
        #第一层输入门
        self.W_i11 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_i12 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_c11 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_c12 = nn.Linear(n_hidden, n_hidden, bias=False)
        #第一层输出门
        self.W_o11 = nn.Linear(emb_size, n_hidden, bias=False)
        self.W_o12 = nn.Linear(n_hidden, n_hidden, bias=False)
        #第一层
        ...
        #第二层遗忘门
        self.W_f21 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_f22 = nn.Linear(n_hidden, n_hidden, bias=False)
        #第二层输入门
        self.W_i21 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_i22 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_c21 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_c22 = nn.Linear(n_hidden, n_hidden, bias=False)
        #第二层输出门
        self.W_o21 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.W_o22 = nn.Linear(n_hidden, n_hidden, bias=False)
        self.tanh = nn.Tanh()
        ...
        self.W_y = nn.Linear(n_hidden, n_class, bias=False)
        self.b_y = nn.Parameter(torch.ones([n_class]))
    ...
    def forward(self, X):
        ...
        y_output1 = []
        c_t1 = torch.randn(sample_size, n_hidden)
        h_t1 = torch.randn(sample_size, n_hidden)
        #重新写的循环
        for x in X:
            #遗忘门
            f_t1 = torch.sigmoid(self.W_f11(x) + self.W_f12(h_t1) + self.b_f1)
            #输入门
            i_t1 = torch.sigmoid(self.W_i11(x) + self.W_i12(h_t1) + self.b_i1)
            c_tv1 = self.tanh(self.W_c11(x) + self.W_c12(h_t1) + self.b_c1)
            #记忆更新，遗忘无用信息并过滤噪声信息
            c_t1 = c_t1 * f_t1 + i_t1*c_tv1
            # 输出门
            o_t1 = torch.sigmoid(self.W_o11(x) + self.W_o12(h_t1) + self.b_o1)
            #把c_t转换成状态
            h_t1 = o_t1*self.tanh(c_t1)
            y_output1.append(h_t1)

        c_t2 = torch.randn(sample_size, n_hidden)
        h_t2 = torch.randn(sample_size, n_hidden)
        for y in y_output1:
            #遗忘门
            f_t2 = torch.sigmoid(self.W_f21(y) + self.W_f22(h_t2) + self.b_f2)
            #输入门
            i_t2 = torch.sigmoid(self.W_i21(y) + self.W_i22(h_t2) + self.b_i2)
            c_tv2 = self.tanh(self.W_c21(y) + self.W_c22(h_t2) + self.b_c2)
            #记忆更新，遗忘无用信息并过滤噪声信息
            c_t2 = c_t2 * f_t2 + i_t2*c_tv2
            # 输出门
            o_t2 = torch.sigmoid(self.W_o21(y) + self.W_o22(h_t2) + self.b_o2)
            #把c_t转换成状态
            h_t2 = o_t2*self.tanh(c_t2)

        model_output = self.W_y(h_t2) + self.b_y
        '''end'''
        return model_output
